...
Friend Class IniFile
    ...
    Friend Sub LoadFromMemory(ByVal sStream As MemoryStream, Optional ByVal bMerge As Boolean = False)
        If Not bMerge Then
            RemoveAllSections()
        End If
        '  Clear the object... 
        Dim tempsection As IniSection = Nothing
        Dim oReader As New StreamReader(sStream)
    ...
    Friend Sub Load(ByVal sFileName As String, Optional ByVal bMerge As Boolean = False)
        If Not bMerge Then
            RemoveAllSections()
        End If
        '  Clear the object... 
    ...
    Friend Sub Save(ByVal sFileName As String)
    ...
    Friend Function SavetoByte() As Byte()
        Dim p As New StringBuilder
        For Each s As IniSection In Sections
            Trace.WriteLine(String.Format("Writing Section: [{0}]", s.Name))
            p.AppendLine(String.Format("[{0}]", s.Name))
            For Each k As IniSection.IniKey In s.Keys
                If k.Value <> String.Empty Then
                    Trace.WriteLine(String.Format("Writing Key: {0}={1}", k.Name, k.Value))
                    p.AppendLine(String.Format("{0}={1}", k.Name, k.Value))
                Else
                    Trace.WriteLine(String.Format("Writing Key: {0}", k.Name))
                    p.AppendLine(String.Format("{0}", k.Name))
                End If
            Next
        Next
        Return System.Text.UTF8Encoding.UTF8.GetBytes(p.ToString)
    End Function


    ' Gets all the sections
    Friend ReadOnly Property Sections() As System.Collections.ICollection
        Get
    ...
    Friend Function AddSection(ByVal sSection As String) As IniSection
    ...
    Friend Function RemoveSection(ByVal sSection As String) As Boolean
        sSection = sSection.Trim()
        Return RemoveSection(GetSection(sSection))
    End Function

    ' Removes section by object, returns trus on success
    Friend Function RemoveSection(ByVal Section As IniSection) As Boolean
    ...
    Friend Function RemoveAllSections() As Boolean
        m_sections.Clear()
        Return (m_sections.Count = 0)
    End Function

    ' Returns an IniSection to the section by name, NULL if it was not found
    Friend Function GetSection(ByVal sSection As String) As IniSection
        sSection = sSection.Trim()
        ' Trim spaces
        If m_sections.ContainsKey(sSection) Then
    ...
    Friend Function GetKeyValue(ByVal sSection As String, ByVal sKey As String) As String
    ...
    Friend Function SetKeyValue(ByVal sSection As String, ByVal sKey As String, ByVal sValue As String) As Boolean
    ...
    Friend Function RenameSection(ByVal sSection As String, ByVal sNewSection As String) As Boolean
    ...
    Friend Function RenameKey(ByVal sSection As String, ByVal sKey As String, ByVal sNewKey As String) As Boolean
        '  Note string trims are done in lower calls.
        Dim s As IniSection = GetSection(sSection)
        If s IsNot Nothing Then
            Dim k As IniSection.IniKey = s.GetKey(sKey)
    ...
    Friend Function RemoveKey(ByVal sSection As String, ByVal sKey As String) As Boolean
        Dim s As IniSection = GetSection(sSection)
        If s IsNot Nothing Then
    ...
    Friend Class IniSection
        ...
        Friend ReadOnly Property Keys() As System.Collections.ICollection
            Get
        ...
        Friend Function AddKey(ByVal sKey As String) As IniKey
            sKey = sKey.Trim()
        ...
        Friend Function RemoveKey(ByVal sKey As String) As Boolean
            Return RemoveKey(GetKey(sKey))
        End Function

        ' Removes a single key by IniKey object
        Friend Function RemoveKey(ByVal Key As IniKey) As Boolean
        ...
        Friend Function RemoveAllKeys() As Boolean
            m_keys.Clear()
            Return (m_keys.Count = 0)
        End Function

        ' Returns a IniKey object to the key by name, NULL if it was not found
        Friend Function GetKey(ByVal sKey As String) As IniKey
            sKey = sKey.Trim()
            If m_keys.ContainsKey(sKey) Then
        ...
        Friend Function SetName(ByVal sSection As String) As Boolean
            sSection = sSection.Trim()
        ...
        Friend Class IniKey
            ...
            Friend Function GetValue() As String
                Return m_sValue
            End Function

            ' Sets the key name
            ' Returns true on success, fails if the section name sKey already exists
            Friend Function SetName(ByVal sKey As String) As Boolean
                sKey = sKey.Trim()
                If sKey.Length <> 0 Then
